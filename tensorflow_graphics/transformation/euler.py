...
r"""This modules implements TensorfFlow Euler angles utility functions.

The Euler angles are defined using a 3-D vector $$[\theta, \gamma, \beta]^T$$
leading to tensors `[?, ..., ?, 3]`.
"""
...
def from_axis_angle(axis, angle, name=None):
  ...
  Args:
    ...
    name: A name for this op. Defaults to "euler_from_axis_angle".
...
def from_quaternion(quaternions, name=None):
  ...
  Args:
    quaternions: N-D tensor of shape `[?, ..., ?, 4]`.
    name: A name for this op. Defaults to "euler_from_quaternion".
  ...
  def gimbal_lock(r01, r02, r20, eps_addition):
    ...
    theta_y = -sign_r20 * tf.constant(m.pi / 2.0, dtype=r20.dtype)
...
def from_rotation_matrix(rotation_matrix, name=None):
  ...
  Note: There is an infinite number of solutions to this problem. There are
  Gimbal locks when abs(rotation_matrix(2,0)) == 1, which are not handled.

  Args:
    rotation_matrix: N-D tensor of shape `[?, ..., ?, 3, 3]`.
    name: A name for this op. Defaults to "euler_from_rotation_matrix".
...
def inverse(euler_angle, name=None):
  ...
  Args:
    euler_angle: N-D tensor of shape `[?, ..., ?, 3]`.
    name: A name for this op. Defaults to "euler_inverse".

  Returns:
    A N-D tensor of shape `[?, ..., ?, 3]` representing Euler angles.
