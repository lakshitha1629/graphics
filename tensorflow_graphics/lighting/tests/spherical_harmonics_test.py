...
class SphericalHarmonicsTest(test_case.TestCase):
  ...
  def test_evaluate_legendre_polynomial_exceptions_x_raised(self):
    ...
    x = np.random.uniform(1.0 + asserts.select_eps_for_addition(tf.float32),
                          2.0)
    with self.assertRaises(tf.errors.InvalidArgumentError):
      self.evaluate(spherical_harmonics.evaluate_legendre_polynomial(l, m, x))
  ...
  @parameterized.parameters(
      ("'f' and 'g' differ in their last dimension.", (1,), (2,)),)
  ...
  @parameterized.parameters(
      ("'l' and 'm' have different shapes.", (3,), (2,), (1,), (1,)),
      ("'theta' and 'phi' have different shapes.", (3,), (3,), (1,), (2,)),
      ("'l', 'm', 'theta', and 'phi' must have the same shape in all but the last dimension.",
       (3,), (3,), (2, 1), (2, 1)),
  )
  def test_evaluate_spherical_harmonics_shape_raised(self, error_msg, *shapes):
    """Tests that the shape exceptions are properly raised."""
    self.assert_exception_is_raised(
        spherical_harmonics.evaluate_spherical_harmonics, error_msg, shapes)
  ...
  def test_evaluate_spherical_harmonics_exception_theta_raised(self):
    ...
    theta = np.random.uniform(
        np.pi + sys.float_info.epsilon, 2.0 * np.pi)
    phi = np.random.uniform(0.0, 2.0 * np.pi)
    with self.assertRaises(tf.errors.InvalidArgumentError):
      self.evaluate(
          spherical_harmonics.evaluate_spherical_harmonics(l, m, theta, phi))
    theta = np.random.uniform(-np.pi, 0.0 - sys.float_info.epsilon)
    with self.assertRaises(tf.errors.InvalidArgumentError):
      self.evaluate(
          spherical_harmonics.evaluate_spherical_harmonics(l, m, theta, phi))
  ...
  def test_evaluate_spherical_harmonics_exception_phi_raised(self):
    ...
    phi = np.random.uniform(
        2.0 * np.pi + sys.float_info.epsilon, 4.0 * np.pi)
    with self.assertRaises(tf.errors.InvalidArgumentError):
      self.evaluate(
          spherical_harmonics.evaluate_spherical_harmonics(l, m, theta, phi))
    phi = np.random.uniform(
        -2.0 * np.pi, 0.0 - sys.float_info.epsilon)
    with self.assertRaises(tf.errors.InvalidArgumentError):
      self.evaluate(
          spherical_harmonics.evaluate_spherical_harmonics(l, m, theta, phi))
  ...
  def test_evaluate_spherical_harmonics_not_raised(self, *shape):
    ...
    self.assert_exception_is_not_raised(
        spherical_harmonics.evaluate_spherical_harmonics, shape)
  ...
  def test_evaluate_spherical_harmonics_preset(self):
    ...
    pt_spherical = math.cartesian_to_spherical_coordinates(pt_3d)
    _, theta, phi = tf.unstack(pt_spherical, axis=-1)
    ones = tf.ones_like(z)
    with self.subTest(name="l_0_m_0"):
      l = tf.zeros_like(theta)
      m = tf.zeros_like(theta)
  ...
  @parameterized.parameters(
      ("'theta' and 'phi' have different shapes.", (4,), (5, 4), (5, 1)),)
  ...
  @parameterized.parameters(
      ("'coefficients' must have 1 dimension.", (2, 2)),)
