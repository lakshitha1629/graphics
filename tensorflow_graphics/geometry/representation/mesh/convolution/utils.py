...
def flatten_batch_to_2d(data, sizes=None, name=None):
  ...
  with tf.compat.v1.name_scope(name, "utils_flatten_batch_to_2d",
    ...
    if sizes is None:
      ...
      mask.set_shape(mask.shape.with_rank(data_ndims - 1))
      flat = tf.boolean_mask(tensor=data, mask=mask)
      def unflatten(flat):
        """Invert flatten_batch_to_2d."""
        # Flattening padded data is implemented with boolean_mask above,
        # so the gradient of boolean_mask can be used to implement the
        # corresponding unflattening.
        flat = tf.convert_to_tensor(value=flat)
        with tf.GradientTape() as g:
          shape = tf.concat([tf.shape(input=mask), tf.shape(input=flat)[-1:]],
                            axis=0)
          dummy = tf.zeros(shape=shape, dtype=flat.dtype)
          g.watch(dummy)
          m = tf.boolean_mask(tensor=dummy, mask=mask) * flat
          return g.gradient(m, dummy)
